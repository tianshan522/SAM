...
class AgeTransformer(object):
	...
	def add_aging_channel(self, img):
		...
		age_channel = torch.full((1, img.shape[1], img.shape[2]), target_age,
		                         dtype=img.dtype, device=img.device)
		img = torch.cat((img, age_channel))
		return img

	def __get_target_age(self):
		if self.target_age == "uniform_random":
			return np.random.randint(low=0., high=101, size=1)[0]
		elif self.target_age == "baby_biased":
			if np.random.random() < 0.6:
				return np.random.randint(low=0, high=6, size=1)[0]
			else:
				return np.random.randint(low=0, high=101, size=1)[0]
		else:
