#!/usr/bin/env python3
"""
提交完整训练任务到 DLP 云平台，并实时流式打印日志。

前提：已运行 login.ps1 完成登录，且 dlp_config.py 中配置正确。

用法:
    conda run -p .conda\sam python scripts/local/cloud/train.py
"""
import sys
import time
from pathlib import Path

# 读取用户配置（scripts/local/dlp_config.py）
_CONFIG_DIR = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_CONFIG_DIR))
try:
    import dlp_config
except ImportError:
    print("错误：找不到 scripts/local/dlp_config.py，请先参考该文件的模板创建并填写配置。")
    sys.exit(1)

if not dlp_config.CLOUD_REPO_DIR or dlp_config.CLOUD_REPO_DIR == "/cloud/tmp/your_name/SAM":
    print("错误：请在 scripts/local/dlp_config.py 中将 CLOUD_REPO_DIR 修改为您的实际云端路径。")
    sys.exit(1)

# 强制使用 Linux 路径分隔符（脚本在 Windows 上运行，目标是 Linux 云端）
CLOUD_REPO_DIR = dlp_config.CLOUD_REPO_DIR.replace("\\", "/")
CLOUD_EXP_DIR  = f"{CLOUD_REPO_DIR}/experiments/adult_to_baby_finetune"
